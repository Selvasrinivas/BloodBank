...
export class BloodBankService {
  ...
  getState(): Observable<any> {
    ...
    return this.http.get<IAddress[]>(environment.bloodUrl + "/address/state", { headers });
  ...
  getArea(state): Observable<any> {
    ...
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.token);
    return this.http.get<any>(environment.bloodUrl+"/address/area/"+state, { headers })
  }
  ...
  getPincode(area): Observable<any> {
    ...
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.token);
    return this.http.get<any>(environment.bloodUrl+"/address/pincode/"+area, { headers })
  }
  ...
  public checkAvailability(bloodgroup :string,pincode:number): Observable<any> {
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.token);
    return this.http.get<IUser[]>(environment.bloodUrl + "/request/"+bloodgroup+"/"+pincode, { headers });
  }
  ...
  public postRequirement(request:IBloodRequest,username:string): Observable<any> {
    ...
    console.log(headers);
    return this.http.post(environment.bloodUrl + "/request/"+username,request,{ headers });
  }

  public getBloodRequests(): Observable<any> {
    // let headers = new HttpHeaders();
    // headers = headers.set('Authorization', 'Bearer ' + this.token);
    return this.http.get<IBloodRequest[]>(environment.bloodUrl + "/request");
  }
  ...
  public getHospitalList(pincode:number): Observable<any> {
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.token);
    return this.http.get<IHospital[]>(environment.bloodUrl + "/slot/hospital/"+pincode , { headers });
  }

  public getNoOfDonors(hospitalname:string,date:Date,time:Time): Observable<any> {
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.token);
    return this.http.get<IHospital[]>(environment.bloodUrl + "/slot/"+hospitalname+"/"+date+"/"+time , { headers });
  }
  ...
  public postSlot(slot:ISlot,hospitalname:string): Observable<any> {
    ...
    console.log("tokennnnn"+this.token)
    console.log(headers);
    return this.http.post(environment.bloodUrl + "/slot/"+hospitalname,slot,{ headers });
  }
  ...
  public postFeedBack(feedBack:IExpSharing,username:string): Observable<any> {
    ...
    console.log("tokennnnn"+this.token)
    console.log(headers);
    return this.http.post(environment.bloodUrl + "/experience/"+username,feedBack, { headers });
  }

  public getExperience(): Observable<any> {
    return this.http.get<IExpSharing[]>(environment.bloodUrl + "/experience");
  }
